...
class LeaderBoard extends React.Component {
  getNumAskedQuestions = user => {
    return Object.keys(this.props.questions).filter(
      key => this.props.questions[key].author === user.id
    ).length;
  };
...
const mapStateToProps = ({ questions, users }) => {
  const orderedUsers = Object.keys(users)
    .sort((aKey, bKey) => {
      ...
      const nAuthorA = Object.keys(questions).filter(
        key => questions[key].author === aKey
      ).length;
      const nAuthorB = Object.keys(questions).filter(
        key => questions[key].author === bKey
      ).length;
